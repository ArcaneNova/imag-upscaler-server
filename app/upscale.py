...
def get_model(scale: int = 2):
    ...
    if model_key not in _models:
        try:
            ...
            # Initialize model based on scale
            if scale == 2:
...
def run_upscale(
    ...
    try:
        ...
        with Image.open(input_path) as img:
            ...
            logger.info(f"Input image: {original_size}, mode: {original_mode}")
            
            # Preprocess image
            processed_img = preprocess_image(img, max_dimension)
            ...
            upscaled_img = model.predict(processed_img)
            ...
            logger.info(f"Image saved: {original_size} -> {final_size}")
            logger.info(f"Memory usage: {start_memory}% -> {final_memory}%")
            
            # Force garbage collection
            del upscaled_img, processed_img
            gc.collect()
